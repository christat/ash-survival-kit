...
fn compile_shader(compiler_cmd: &str, input_file: &str, output_file: &str, err_msg: &str) {
    ...
    Command::new(compiler_cmd)
        ...
        .arg("-o")
        .arg(script_dir.join(Path::new("src/shaders")).join(output_file).to_str().unwrap())
        .output()
        .expect(err_msg);
}
...
fn main() {
    ...
    compile_shader(&compiler_cmd, "shader.frag", "frag.spv", "Failed to compile fragment shader!");
}